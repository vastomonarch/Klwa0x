...
// ðŸŒŸ SECURE FIREBASE URL FROM GITHUB SECRETS ðŸŒŸ
...
async function startBot() {
    if (!FIREBASE_URL) {
        console.log("âŒ ERROR: FIREBASE_URL is missing in GitHub Secrets!");
        process.exit(1);
    }
    ...
    sock.ev.on('connection.update', (update) => {
        ...
        if (qr) {
            console.clear(); 
            console.log('\n==================================================');
            console.log('âš ï¸ QR CODE TOO BIG? CLICK "View raw logs" in top right!');
            console.log('==================================================\n');
            qrcode.generate(qr, { small: true }); 
        }

        if (connection === 'open') console.log('âœ… JAVAGOAT AI IS ONLINE!');
        if (connection === 'close') {
    ...
    sock.ev.on('messages.upsert', async (m) => {
        ...
        // --- ðŸ›’ STEP 2: FINISH ORDER & SEND TO ADMIN PANEL ---
        if (orderStates[sender]?.step === 'WAITING_FOR_ADDRESS') {
            ...
            await sock.sendMessage(sender, { text: `âœ… *Order Placed Successfully!* \n\nThank you! Your order for *${item.name}* is being prepared. \n\n*Total:* â‚¹${javaGoatOrder.total} (Inc. Delivery)\n*Status:* Preparing\n\nWe will deliver it to your address soon.` });
            delete orderStates[sender]; 
            return;
        }

        // --- ðŸŒŸ STEP 1: START ORDER FLOW (WITH IMAGE & PHONE REQUEST) ---
        if (text.startsWith("order ")) {
            ...
            if (!matchedItem) {
                await sock.sendMessage(sender, { text: `âŒ Sorry, we couldn't find *${productRequested}* in our menu today.\n\nType *menu* to see all available items.` });
                return;
            }

            orderStates[sender] = { step: 'WAITING_FOR_ADDRESS', item: matchedItem };
            
            // ðŸŒŸ NEW: SEND PRODUCT IMAGE + ASK FOR PHONE NUMBER ðŸŒŸ
            const captionText = `ðŸ›’ *Order Started!* \n\nYou selected: *${matchedItem.name}* (â‚¹${matchedItem.price})\n\nPlease reply with your *Full Name, Phone Number, and Delivery Address*.`;
            ...
            await sock.sendMessage(sender, { text: "ðŸ›’ *How to order:* \nPlease type 'order' followed by the dish name. \nExample: *order pizza*" });
        }
        ...
        // --- DYNAMIC MENU FEATURE ---
            ...
            let menuMessage = "ðŸ” *JAVAGOAT LIVE MENU* ðŸ•\n\n";
            currentMenu.forEach(item => {
                menuMessage += `ðŸ”¸ *${item.name}* - â‚¹${item.price}\n`;
            });
